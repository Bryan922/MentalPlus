document.addEventListener('DOMContentLoaded', function() {
    // Configuration des horaires de nuit
    const CONFIG = {
        startHour: 20,    // Début à 20h
        endHour: 6,       // Fin à 6h le lendemain
        slotDuration: 60, // Durée d'un RDV en minutes
        breakDuration: 30, // Pause entre les RDV en minutes
        nightRate: 80,    // Tarif horaire de nuit en euros
        excludedDays: []  // Pas de jours exclus pour le service de nuit
    };
    ...
    function initializeTimeSlots() {
        ...
        // Ajouter les créneaux de 20h à 23h
        for (let hour = CONFIG.startHour; hour < 24; hour++) {
            addTimeSlot(hour);
        }

        // Ajouter les créneaux de 00h à 6h
        for (let hour = 0; hour < CONFIG.endHour; hour++) {
            addTimeSlot(hour);
        }
    }

    // Ajouter un créneau horaire
    function addTimeSlot(hour) {
        const slotDiv = document.createElement('div');
        slotDiv.className = 'time-slot';
        slotDiv.textContent = `${hour.toString().padStart(2, '0')}:00`;
        document.querySelector('.time-slots').appendChild(slotDiv);
    }
    ...
    function isSlotAvailable(time, appointments) {
        ...
        // Vérifier si l'heure est dans les horaires de nuit
    ...
    function renderAppointments(appointments) {
        ...
        appointments.forEach(appointment => {
            ...
            // Ajuster la position en fonction de l'heure (avant ou après minuit)
            const adjustedHour = hours >= CONFIG.startHour ? hours - CONFIG.startHour : hours + (24 - CONFIG.startHour);
            const top = adjustedHour * (60 + CONFIG.breakDuration) + minutes;
            ...
            appointmentEl.innerHTML = `
                ...
                <div class="appointment-price">Tarif : ${CONFIG.nightRate}€</div>
            `;
    ...
    function updateStatistics(appointments) {
        ...
        const totalRevenue = confirmedAppointments.length * CONFIG.nightRate;
        
        document.querySelector('.stats-grid .stat-value').textContent = appointments.length;
        document.querySelector('.stats-grid .stat-value:nth-child(2)').textContent = `${totalRevenue}€`;
    }
    ...
    function openNewAppointmentForm() {
        ...
        alert(`Création d'un nouveau rendez-vous de nuit\nTarif : ${CONFIG.nightRate}€/heure\nCréneaux disponibles : ` + 
    ...
    function handleFilters() {
        ...
        filterBtn.addEventListener('click', () => {
            alert('Filtres du planning de nuit');
        });
    }
