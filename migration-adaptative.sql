-- Script de Migration Adaptative pour MentalSerenity
...
-- Table des rendez-vous (avec vérification de structure)
DO $$
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'appointments') THEN
        ...
        RAISE NOTICE 'Table appointments créée avec appointment_date';
    ELSE
        -- Vérifier si la colonne appointment_date existe
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'appointments' AND column_name = 'appointment_date') THEN
            ...
            IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'appointments' AND column_name = 'date') THEN
                -- Renommer date en appointment_date
                ALTER TABLE appointments RENAME COLUMN date TO appointment_date;
                RAISE NOTICE 'Colonne date renommée en appointment_date';
            ELSE
                -- Ajouter la colonne appointment_date
                ALTER TABLE appointments ADD COLUMN appointment_date DATE;
                RAISE NOTICE 'Colonne appointment_date ajoutée';
            END IF;
        END IF;
        
        -- Ajouter les colonnes manquantes si elles n'existent pas
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'appointments' AND column_name = 'heure') THEN
            ALTER TABLE appointments ADD COLUMN heure TIME;
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'appointments' AND column_name = 'duree') THEN
            ALTER TABLE appointments ADD COLUMN duree INTEGER DEFAULT 60;
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'appointments' AND column_name = 'domaine') THEN
            ALTER TABLE appointments ADD COLUMN domaine VARCHAR(100);
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'appointments' AND column_name = 'status') THEN
            ALTER TABLE appointments ADD COLUMN status VARCHAR(20) DEFAULT 'confirmé';
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'appointments' AND column_name = 'motif') THEN
...
-- Afficher la structure de la table appointments
SELECT 
    column_name,
    data_type,
    is_nullable
